[Home](../README.md) › Services

# Services

This category documents all self-hosted services running in the homelab.

---

## Service Index

| Service | Host | Internal URL | Deployment | Status |
|---|---|---|---|---|
| [GitLab CE](gitlab.md) | `erebor.shire` (10.136.20.102) | `http://10.136.20.102` | Proxmox VM (Debian 12) | ✅ Running |
| [Grafana](grafana.md) | `palantir.shire` (10.136.20.105) | `http://10.136.20.105:3000` | Proxmox VM (Debian 12) | ✅ Running |
| [Prometheus](prometheus.md) | `palantir.shire` (10.136.20.105) | `http://10.136.20.105:9090` | Proxmox VM (Debian 12) | ✅ Running |
| [Heimdall](heimdall.md) | `thal.shire` (10.136.20.104) | `http://10.136.20.104` | Proxmox VM (Debian 12) | ✅ Running |
| [n8n](n8n.md) | `radagast.shire` (10.136.20.101) | `http://10.136.20.101:5678` | Proxmox VM (Debian 12) | ✅ Running |
| [Home Assistant](home-assistant.md) | `weathertop.shire` (10.136.20.107) | `http://10.136.20.107:8123` | Proxmox VM (Home Assistant OS) | ✅ Running |
| [LLM Server](llm-server.md) | `valinor.shire` (10.136.20.20) | `http://10.136.20.20:3000` | Mac Mini M4 (Docker + LM Studio) | ✅ Running |
| [Future Services](future-services.md) | — | — | Planned | 📋 Planned |
